...
export default function VoiceCoachPage() {
  ...
  const labels: Record<string, { title: string; subtitle: string }> = {
    background: {
      ...
      subtitle: "Erzähl kurz von deinem Weg.",
    },
    current: {
      title: "Aktuelle Rolle",
      subtitle: "Was machst du gerade beruflich?",
    },
    goals: {
      title: "Ziele & Interessen",
      subtitle: "Wohin soll es als Nächstes gehen?",
    },
  };
  const active = (field && labels[field]) || null;
      ...
      >
        <header className="w-full text-center space-y-3">
          ...
          <h1 className="text-3xl font-semibold">
            {active ? active.title : "Sprachaufnahme"}
          </h1>
          <p className="text-sm text-white/70">
            {active ? active.subtitle : "Hier kannst du deine Stimme aufnehmen."} Tippe auf den Kreis, um zu starten, und erneut,
